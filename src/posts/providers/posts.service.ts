...
export class PostsService {
  ...
  public async createPost(@Body() createPostDto: CreatePostDto) {
    ...
    try {
      ...
      throw new BadRequestException(`User with id ${createPostDto.authorId} does not exist`);
    }

    try {
      tags = await this.tagsService.findMultipleTags(createPostDto.tags)
    } catch (error) {
      throw new BadRequestException(`Tag with id ${createPostDto.tags} does not exist`);
    }
    ...
    try {
      ...
      throw new BadRequestException(`Error while saving post to the DB: ${error.message}`);
    }
  ...
  public async updatePost(patchPostDto: PatchPostDto){
    ...
    try {
      ...
      throw new RequestTimeoutException(`Tag with id ${patchPostDto.tags} does not exist`);
    }

    if (!tags) {
      throw new BadRequestException(`Tag with id ${patchPostDto.tags} does not exist`);
    }
    ...
    try {
      ...
      throw new BadRequestException(`Post with id ${patchPostDto.tags} does not exist`);
    }
